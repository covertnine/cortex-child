...
$cortex_display_read_more_text        = get_sub_field('read_more_text');
...
<section id="section-<?php echo $cortex_counter; ?>" class="magazine_latest<?php if (! empty($cortex_customClass)) {
    ...
    <div class="section-bg"<?php cortex_section_style($cortex_background, $cortex_backgroundColor, $cortex_backgroundImage, $cortex_backgroundPattern, $cortex_backgroundRepeat); ?>>
        <div class="container">
    ...
    ?>
            <div class="row wow animated fadeInUp">
                <div class="magazine_latest_title col-md-12 mar30B">
                    <?php if ($cortex_title != '') {
                        $contents = explode(' ', $cortex_title);
                            $spannercounter = 1;
                            $spanned_content = '';
                            foreach ($contents as $c){
                                $spanned_content .= '<span class=word-'.$spannercounter.'>'.$c.'</span> ';
                                $spannercounter++;
                            }
        echo '<span class="h2 center mar10B"><span>' . $spanned_content . '</span>';
    } ?>
                    <?php if ($cortex_sub_title != '') {
        echo '<span class="subtitle center mar10B">' . $cortex_sub_title . '</span>';
    } ?>
                </div>
            </div>
            <?php
            ...
            <div class="row">
                ...
                if (!empty($cortex_filter_by_post_type)) {
                    if (post_type_exists($cortex_filter_by_post_type)) {
                        $cortex_custom_post_type = $cortex_filter_by_post_type;
                    } else {
                        echo "<h5>The post type you entered (<i>" . esc_html($cortex_filter_by_post_type) . "</i>) does not exist</h5>";
                        return;
                    }
                } else {
                    $cortex_custom_post_type = 'post';
                }
                    $args = array(
                        'post_type'              => $cortex_custom_post_type,
                        'post_status'    => 'publish',
                        'pagination'             => false,
                        'orderby'     => 'date',
                        'order'      => 'DESC',
                        'posts_per_page'   => $cortex_number_of_posts,
                    );

                // filter by category
                if (!  empty($cortex_filter_by_category)) {
                    $args['category__in'] = $cortex_filter_by_category;
                }
...
?>
                ...
                <div class="<?php if ($cortex_sidebar == 'sidebar-none') {
    echo 'col-md-12 ';
} else {
    echo 'col-xs-12 col-sm-12 col-md-9 ';
} ?><?php if ($cortex_sidebar == 'sidebar-left') {
    echo ' col-md-push-3';
                    ...
                    if ($cortex_wp_query->have_posts()) {
                        ...
                        switch ($cortex_columns) {
                            ...
                            break;
                            case 2:
                           $cortex_column_classes = "col-xs-12 col-sm-6 col-md-6";
                            break;
                            default:
                            $cortex_column_classes = "col-xs-12 col-sm-4 col-md-4";
                        } 
                    ...
                    while ($cortex_wp_query->have_posts()) {
                        ...
                        if (($cortex_format === false) || ($cortex_format == 'image')) {
                            $cortex_format = 'standard';
                        } ?>
                        <div class="<?php echo $cortex_column_classes ?>">
                               ...
                               <?php }
                                if ($cortex_format != 'quote') { // check for post quote type
                                    ...
                                    if ($cortex_display_post_excerpt == true) { // check if excerpt is to be displayed?>
                                        ...
                                        <div class="entry-content magazine-article-content">
                                            <?php echo cortex_the_excerpt('', 25, 1); ?>
                                        </div>
                    ...
                    <?php
                    if ($i % $cortex_columns == 0) { // check for the article count to clear appropriately
                    ?>
                    ...
                    if ($i % 2 == 0) { // now check for the sm column layouts
                        ...
                        <?php if ($cortex_view_more_btn == true) {
                            if ( empty($cortex_view_more_button_text) ) {
                                $cortex_view_more_button_text = "View All";
                            } 
                        ?>
                        <div class="row wow animated fadeInUp">
                            <div class="col-md-12">
                            <div class="view-more-btn center mar30B">
                                <a class="btn btn-md btn-default secondary-color-bg" href="<?php echo $cortex_view_more_button_link; ?>"><?php esc_html_e($cortex_view_more_button_text, 'cortex'); ?></a>
                            </div>
                            </div>
                        </div><!--end row-->
                        <?php
...
?>
    ...
    ?>
                <div class="col-xs-12 col-sm-12 col-md-3 wow animated fadeInUp<?php if ($cortex_sidebar == 'sidebar-left') {
        echo ' col-md-pull-9';
    } ?>">
