			...
			<section id="section-events" class="event-listing">
				...
				if ( $cortex_events_page_header_type == 'background-next-event' ) { // the heading is made up of fields from the next upcoming event
					...
					if ( empty( $cortex_limit_by_category ) ) { // query all events
						...
						// The Query for the next featured event
					...
					if ( $cortex_query->have_posts() ) {
						while ( $cortex_query->have_posts() ) {
							...
							$cortex_u_start      = get_field( 'date_and_time', false, false );
							$cortex_u_date       = new DateTime( $cortex_u_start );
					...
					<div class="row wow animated fadeInUp">
						<?php if ( $cortex_show_next_event_featured == true ) { ?>
							...
							<div class="events-header-text next-event">
								<?php if ( ! empty( $cortex_next_event_heading ) ) { ?><h5 class="center secondary-font light"><span class="rf-date"><?php echo $cortex_next_event_heading;?></span></h5><?php } ?>
								...
								<?php if ( ! empty( $cortex_u_location_city_country ) ) { ?>
									<h5 class="center mar30B secondary-font">
										<?php if ( ! empty( $cortex_u_date ) ) { ?><span class="u-date accent-color-text light"><?php echo $cortex_u_date->format('l, F j, Y'); ?><span class="dot headline-color-text"> &middot; </span></span><?php } ?>
			...
			?>
				<header id="section-events-heading" class="events-page-header" data-top="opacity: 1" data-top-bottom="opacity: 0" data-anchor-target="#section-events-heading">
					<div class="container">
						<div class="row">
							<div class="col-xs-12">
								<h1 class="center"><span class="rf-date">Upcoming Shows</span></h1>
							</div>
							</div>
								</div>
							</header>
		...
		if ( $cortex_query->have_posts() ) {
			...
			?>
				...
				<?php while ( $cortex_query->have_posts() ) {
					...
					$cortex_date 			 = $cortex_date_setup->format('l, F j, Y');
...
?>
			...
			?>
						<tr class="event-basic-row wow animated fadeInUp">
							<td class="event-basic-cell date h6 headline-font accent-color-bg"><span class="day secondary-font"><?php echo $cortex_day; ?></span><span class="date-number"><?php echo $cortex_date; ?></span><span class="time"><?php echo $riot_time; ?></span>

							</td>
							<td class="event-basic-cell headline h5 headline-font">
								<?php if ( $cortex_event_headline != get_the_title() ) { ?><a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><em><?php the_title(); ?></em></a><?php } ?>
		...
		?>
			...
			while ( $cortex_old_query->have_posts() ) {
				...
				$cortex_o_start    	     = get_field( 'date_and_time', false, false );
				$cortex_o_date_setup       = new DateTime( $cortex_o_start );
				$cortex_o_day				 = $cortex_o_date_setup->format('l');
				$cortex_o_date 			 = $cortex_o_date_setup->format('m.j.y');
