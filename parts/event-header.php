...
$cortex_u_date_setup      		 = new DateTime( $cortex_u_start );
$cortex_u_date      			 = $cortex_u_date_setup->format('F j, Y');
...
if ( ( empty( $cortex_featured_header ) ) && ( $cortex_theme_options['cortex_theme_style'] == 'light') ) {$cortex_color_switch = ' dark-color-text';}
		...
		<div class="row">
			...
			<?php } ?>
				<div class="events-header-text next-event">
					<?php if ( ! empty( $cortex_event_headline ) ) { ?><h1 class="h5 center secondary-font light<?php if ( ! empty( $cortex_color_switch ) ) { echo $cortex_color_switch;} ?>"><span class="rf-date"><?php echo $cortex_event_headline;?></span></h1><?php } ?>
